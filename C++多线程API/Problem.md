Thread_1:存在主进程执行完子进程未执行完就提前释放资源的结果，而且存在“输出两个换行”，这种情况就是比如输出Main Display！并输出endl后，时间片用完，轮到子线程占用CPU，子进程上一次时间片用完时停在了Fun Display!,下一次时间片过来时，刚好开始输出endl，此时就会“输出两个换行”。
Thread_2:仍然存在输出两个换行。
